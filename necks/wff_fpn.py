...
class SeparableConv2d(nn.Layer):
    """ Separable Convolution as used in BiFPN """
...
class WFFFPN(nn.Layer):
    def __init__(self, in_channels, out_channels, use_asf=False,epsilon=1e-4, **kwargs):
        ...
        # self.psaf = psaf(self.out_channels, self.out_channels)
        # self.rseconv = RSELayer(self.out_channels)
        ...
        self.dsaf_2=DSAF(in_channels=in_channels[0], out_channels=self.out_channels)
        self.dsaf_3=DSAF(in_channels=in_channels[1], out_channels=self.out_channels)
        self.dsaf_4=DSAF(in_channels=in_channels[2], out_channels=self.out_channels)
        self.dsaf_5=DSAF(in_channels=in_channels[3], out_channels=self.out_channels)
        self.dsaf = DSAF(in_channels=self.out_channels, out_channels=self.out_channels)
    ...
    def forward(self, x):
        ...
        f2_out = self._weighted_sum(self.w4, f2, self.downsample(f1_out))
        f2_out = self.sep_conv(f2_out)


        f3_out = self._weighted_sum(self.w4, f3, self.downsample(f2_out))
        f3_out = self.sep_conv(f3_out)

        f4_out = self._weighted_sum(self.w4, f4, self.downsample(f3_out))
        ...
        # avg_attn = paddle.mean(fuse, dim=1, keepdim=True)
        # max_attn, _ = paddle.max(fuse, dim=1, keepdim=True)
        # agg = paddle.cat([avg_attn, max_attn], out_channels=1)
...
class psaf(nn.Layer):
    def __init__(self,inplans, planes):
        super(psaf, self).__init__()
        self.PSA = PSAModule(inplans, planes)
        self.ASF = ASFBlock(inplans, planes)
...
# class psaf1(nn.Layer):
#     def __init__(self):
#         self.PSA = PSAModule(dim=inplans, outdim=planes)
#         self.ASF = ASFBlock(dim=inplans, outdim=planes)
...
class DSAF(nn.Layer):
    def __init__(self, in_channels, out_channels):
        ...
        self.global_branch = nn.Sequential(
            ...
            nn.Conv2D(in_channels, out_channels, kernel_size=1, bias_attr=False),  # 1x1卷积
        ...
        self.local_branch = nn.Sequential(
            nn.Conv2D(in_channels, in_channels, kernel_size=3, padding=1, groups=in_channels, bias_attr=False),  # 深度卷积
            nn.Conv2D(in_channels, out_channels, kernel_size=1, bias_attr=False),  # 逐点卷积
            nn.BatchNorm2D(out_channels),
            nn.ReLU()
        )
